# train_ast_va_fixed_with_dynamic_annotations_mps.py
"""
Train a compact AST-like transformer to predict valence & arousal from precomputed log-mel .npy spectrograms.
Added: dynamic annotation window aggregation (mean + std) controlled by ANNOT_WINDOW_SEC hyperparameter.
Patched: MPS (Apple GPU) device detection and AMP support toggle.
...
STATIC_CSV = "annotations/static_annotations_averaged_songs_1_2000.csv"  # path to CSV annotations (fallback)
# Optional dynamic annotation files/dir. The loader will try the following (in order):
# 1) annotations_dynamic/{stem}_valence.csv and {stem}_arousal.csv
# 2) global CSVs DYNAMIC_VALENCE_CSV / DYNAMIC_AROUSAL_CSV that contain an 'id' column and time/value columns
DYNAMIC_ANNOT_DIR = "dynamicannotations"  # directory containing per-track dynamic annotation CSVs
DYNAMIC_VALENCE_CSV = "valence.csv"
DYNAMIC_AROUSAL_CSV = "arousal.csv"

# Annotation aggregation window (hyperparameter): size in seconds to average annotations over
ANNOT_WINDOW_SEC = 2.0
...
USE_AMP = True            # enable mixed precision (will be overridden to False on non-CUDA devices)
...
# device selection: prefer CUDA, then Apple's MPS (Metal) on macOS, otherwise CPU
if USE_GPU:
    if torch.cuda.is_available():
...
# Mixed precision: enable AMP only when running on CUDA (AMP is not supported on MPS)
USE_AMP = True if device.type == "cuda" else False
print("USE_AMP set to", USE_AMP)

# ---------------- CSV loading & expansion (unchanged) ----------------
...
def _try_load_timeseries_csv(path: Path) -> Optional[Tuple[np.ndarray,np.ndarray]]:
    """Try to load a CSV as a timeseries of (time_seconds, value).
    Accepts column names like ['time','value'] or no headers where first column=time second=value.
    ...
    for c in df.columns:
        low = c.lower()
        if low in ("time","t","seconds","timestamp"):
            time_col = c; break
    if time_col is None and df.shape[1] >= 2:
        time_col = df.columns[0]
    # value column
    for c in df.columns:
        low = c.lower()
        if low in ("value","valence","arousal","v"):
            val_col = c; break
...
def load_global_dynamic_map(valence_csv: str, arousal_csv: str) -> Optional[Dict[str, Tuple[np.ndarray,np.ndarray,np.ndarray,np.ndarray]]]:
    """If global CSVs exist and contain an 'id' column, return mapping id->(t_val,v_val,t_aro,v_aro)
    Otherwise return None."""
    pval = Path(valence_csv)
    paro = Path(arousal_csv)
    if not pval.exists() or not paro.exists():
        return None
    try:
        dfv = pd.read_csv(pval)
        dfa = pd.read_csv(paro)
    except Exception:
        return None
    # require 'id' column
    id_col_v = None
    ...
    for c in dfv.columns:
        if c.lower() in ("id","song_id","track","file","filename"):
            id_col_v = c; break
    for c in dfa.columns:
        if c.lower() in ("id","song_id","track","file","filename"):
            id_col_a = c; break
    if id_col_v is None or id_col_a is None:
        return None
    # build map grouping by id. Expect time column + value column otherwise fail
    out = {}
    for _, row in dfv.iterrows():
        ident = str(row[id_col_v])
        # find time/value columns
        pair = _try_load_timeseries_csv(pval)
        if pair is None:
            continue
        # naive: assume global CSV includes rows for many ids is not supported; we skip sophisticated parsing
    return None


def compute_window_stats(times: np.ndarray, values: np.ndarray, start_t: float, window_sec: float) -> Tuple[float,float]:
    """Compute mean and std of `values` where times in [start_t, start_t+window_sec).
    Returns (mean, std). If no samples found, returns (np.nan, np.nan)."""
    if times is None or values is None:
        return float('nan'), float('nan')
    mask = (times >= start_t) & (times < (start_t + window_sec))
    if not np.any(mask):
        return float('nan'), float('nan')
    seg = values[mask]
    return float(np.nanmean(seg)), float(np.nanstd(seg))
...
class SpectrogramVADataset(Dataset):
    def __init__(self, data_root: str, annotations: Dict[str, Tuple[float,float]], metadata_path: str,
        ...
        self.dynamic_dir = Path(DYNAMIC_ANNOT_DIR)
        self.dynamic_valence_csv = Path(DYNAMIC_VALENCE_CSV)
        self.dynamic_arousal_csv = Path(DYNAMIC_AROUSAL_CSV)
    ...
    def _random_crop(self, x: np.ndarray) -> Tuple[np.ndarray,int]:
        T = x.shape[1]
        if T == self.fixed_frames:
            return x, 0
        if T < self.fixed_frames:
            pad = np.repeat(x[:, -1:], repeats=(self.fixed_frames - T), axis=1)
            return np.concatenate([x, pad], axis=1), 0
        if self.train:
            start = np.random.randint(0, T - self.fixed_frames + 1)
        else:
            start = (T - self.fixed_frames) // 2
        return x[:, start:start + self.fixed_frames], int(start)

    def __getitem__(self, idx: int):
        p = self.files[idx]
        static_va = self._match_annotation(p)
        x = self._load_and_normalize(p)
        x_cropped, start_frame = self._random_crop(x)
        if self.train:
            x_cropped = spec_augment(x_cropped, max_mask_time=0.08, max_mask_freq=0.15)
        # compute target vector: always (mean_val, std_val, mean_aro, std_aro)
        # 1) try dynamic per-track files in DYNAMIC_ANNOT_DIR named {stem}_valence.csv / _arousal.csv
        stem = p.stem
        fps = float(self.sr) / float(self.hop_length)
        clip_start_seconds = float(start_frame) / fps
        target = None
        v_ts = _try_load_timeseries_csv(self.dynamic_dir / f"{stem}_valence.csv")
        a_ts = _try_load_timeseries_csv(self.dynamic_dir / f"{stem}_arousal.csv")
        if v_ts is not None and a_ts is not None:
            t_val, v_val = v_ts
            t_aro, v_aro = a_ts
            mean_v, std_v = compute_window_stats(t_val, v_val, clip_start_seconds, ANNOT_WINDOW_SEC)
            mean_a, std_a = compute_window_stats(t_aro, v_aro, clip_start_seconds, ANNOT_WINDOW_SEC)
            target = np.array([mean_v, std_v, mean_a, std_a], dtype=np.float32)
        else:
            # fallback: use static annot (val, aro) and set std to 0
            mean_v, mean_a = static_va
            target = np.array([float(mean_v), 0.0, float(mean_a), 0.0], dtype=np.float32)
        return x_cropped.astype(np.float32), torch.tensor(target, dtype=torch.float32)

# ---------------- augmentations / patches / model (unchanged except head dim) ----------------
...
class PatchTransformer(nn.Module):
    ...
    def forward(self, patches: torch.Tensor, padding_mask: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.pos_embed.shape[1] < (N + 1):
            raise RuntimeError(f"pos_embed length {self.pos_embed.shape[1]} too small for N+1={N+1}. Increase max_patches when constructing model.")
        x = x + self.pos_embed[:, : (N + 1), :].to(x.device)
        if padding_mask is not None:
            cls_mask = torch.zeros((B, 1), dtype=torch.bool, device=x.device)
            src_key_padding_mask = torch.cat([cls_mask, padding_mask.to(x.device)], dim=1)
        else:
...
class ASTRegressor(nn.Module):
    def __init__(self, patch_area: int, max_patches: int = 256):
        ...
        self.head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(512, 4)  # now output 4 values: mean_val, std_val, mean_aro, std_aro
        )
...
def collate_fn(batch):
    ...
    patches_tensor = torch.stack(padded, dim=0)
    padding_mask = torch.stack(padding_masks, dim=0)
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        if preds_cat.shape[0] > 1:
            try:
                ...
                p_val = float("nan")
            try:
                p_aro = np.corrcoef(preds_cat[:,2], trues_cat[:,2])[0,1]
            except Exception:
                p_aro = float("nan")
        else:
        ...
        if val_loss < best_val:
            best_val = val_loss
            torch.save(model.state_dict(), "best_ast_va_pt_win.pt")
            print("Saved best model -> best_ast_va_pt_win.pt")

...
def plot_valence_arousal_scatter(
    ...
    if values.shape[1] != 2:
        raise ValueError("Each element must be (valence, arousal)")
    ...
    plt.xlabel("Valence")
    plt.ylabel("Arousal")
    plt.title("Model Valence vs Arousal Output over Training Data (2D Scatter)")
...
if __name__ == "__main__":
    ...
    for path in mp3_files:
        ...
        graph.append((np.mean(valence_list), np.mean(arousal_list)))
        dic[song_title] = (float(np.mean(valence_list)), float(np.mean(arousal_list)))

    with open("songs.json", "w") as f:
        json.dump(dic, f, indent=4)

    plot_valence_arousal_scatter(graph)
